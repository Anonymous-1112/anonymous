...
with open("./fig_surgery/fig1_nb101_stat.pkl", "rb") as r_f:
    stat = {k: v for k, v in pickle.load(r_f).items()}

seeds = [20, 2020, 202020]
statss = []
for seed in seeds:
    keep = {k.replace("@", "/"): v for k, v in stat.items() if f"seed{seed}@" in k}
    statss.append(keep)
...
with open("./fig_surgery/fig1_nb101_stat_acc.pkl", "rb") as r_f:
    derived = pickle.load(r_f)

mean_acc = derived["derive"].mean(0)
gt = [x["validation_accuracy"] for x in derived["gt"]]
