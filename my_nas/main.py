...
import my_nas
from my_nas.dataset import AVAIL_DATA_TYPES
from my_nas import btcs, germ, utils, BaseRollout
from my_nas.common import rollout_from_genotype_str
from my_nas.utils.common_utils import _OrderedCommandGroup, _dump, _dump_with_perf
...
def _set_gpu(gpu):
    ...
    if torch.cuda.is_available():
        set_reproducible = bool(os.environ.get("MYNAS_REPRODUCIBLE", False))
        if set_reproducible:
            LOGGER.info("MYNAS_REPRODUCIBLE environment variable set. Disable cudnn.benchmark, "
...
@click.group(cls=_OrderedCommandGroup,
             help="The mynas NAS framework command line interface. "
             "Use `MYNAS_LOG_LEVEL` environment variable to modify the log level.")
@click.version_option(version=my_nas.__version__)
...
@click.option("--vis-dir", default=None, type=str,
              ...
              "need `tensorboard` extra, `pip install my_nas[tensorboard]`")
@click.option("--develop", default=False, type=bool, is_flag=True,
              help="in develop mode, will copy the `my_nas` source files into train_dir for backup")
...
def search(cfg_file, gpu, seed, load, save_every, save_controller_every, interleave_report_every,
    ...
    if vis_dir:
        ...
        try:
            ...
            LOGGER.error("Cannot import module tensorboardX. Will IGNORE the `--vis-dir` option! "
                         "Try installing the dependency manually, or `pip install my_nas[vis]`")
            _writer = None
        else:
    ...
    if train_dir:
        # backup config file, and if in `develop` mode, also backup the my_nas source code
        train_dir = utils.makedir(train_dir, remove=True, quiet=quiet)
        shutil.copyfile(cfg_file, os.path.join(train_dir, "config.yaml"))

        if develop:
            import pkg_resources
            src_path = pkg_resources.resource_filename("my_nas", "")
            backup_code_path = os.path.join(train_dir, "my_nas")
            if os.path.exists(backup_code_path):
                shutil.rmtree(backup_code_path)
            LOGGER.info("Copy `my_nas` source code to %s", backup_code_path)
    ...
    setproctitle.setproctitle("mynas-search config: {}; train_dir: {}; vis_dir: {}; cwd: {}"\
...
def random_sample(cfg_file, out_file, n, gpu, seed, dump_mode, unique):
    ...
    setproctitle.setproctitle("mynas-random-sample cfg: {}; cwd: {}".format(cfg_file, os.getcwd()))
...
def sample(load, out_file, n, save_plot, gpu, seed, dump_mode, prob_thresh, unique):
    ...
    setproctitle.setproctitle("mynas-sample load: {}; cwd: {}".format(load, os.getcwd()))
...
def eval_arch(cfg_file, arch_file, load, gpu, seed, save_plot, save_state_dict, steps,
    ...
    setproctitle.setproctitle("mynas-eval-arch config: {}; arch_file: {}; load: {}; cwd: {}"\
...
def derive(cfg_file, load, out_file, n, save_plot, test, steps, gpu, seed, dump_mode, runtime_save):
    ...
    setproctitle.setproctitle("mynas-derive config: {}; load: {}; cwd: {}"\
...
def mptrain(seed, cfg_file, load, load_state_dict, save_every, train_dir, quiet):
    ...
    setproctitle.setproctitle("mynas-train config: {}; train_dir: {}; cwd: {}"\
...
def test(cfg_file, load, load_state_dict, split, gpus, seed): #pylint: disable=redefined-builtin
    ...
    setproctitle.setproctitle("mynas-test config: {}; load: {}; cwd: {}"\
                              .format(cfg_file, load, os.getcwd()))
