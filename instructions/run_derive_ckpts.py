...
GPUs = [0, 1, 2, 3]
_HOME_DIR = os.path.abspath(os.path.expanduser(os.environ.get("MYNAS_HOME", "~/mynas")))
        ...
        if args.subset:
            ...
            if not args.iso:
                derive_cfg["controller_cfg"]["text_file"] = os.path.join(
                    _HOME_DIR, "mynas/data/nasbench-201/non-isom.txt"
                )
                arch_num = 6466
            else:
                derive_cfg["controller_cfg"]["text_file"] = os.path.join(
                    _HOME_DIR, "mynas/data/nasbench-201/iso.txt"
                )
                arch_num = 15625
    else:
...
def _worker(p_id, gpu_id, queue):
    while 1:
        ...
        if call_derive:
            # call derive
            cmd = (
                "mynas derive {} --load {} --out-file {} --gpu {} -n {} --test --seed 123 "
                "--runtime-save >{} 2>&1"
            ).format(cfg_file, ckpt_dir, out_file, gpu_id, num_arch, derive_log)
        else:
            # call eval-arch
            cmd = (
                "mynas eval-arch {} {} --load {} --dump-rollouts {} --gpu {} --seed 123 "
                ">{} 2>&1"
