...
from my_nas.rollout.base import BaseRollout
from my_nas.controller.base import BaseController
from my_nas.evaluator.base import BaseEvaluator
from my_nas.rollout.compare import CompareRollout
from my_nas.evaluator.arch_network import ArchEmbedder
from my_nas.utils import DenseGraphConvolution, DenseGraphFlow
from my_nas.weights_manager.shared import SharedCell, SharedOp
from my_nas.weights_manager.base import CandidateNet, BaseWeightsManager

import sys
#sys.path.append("/home/Anonymous/nasbench-1shot1")
# please install the lib from https://github.com/automl/nasbench-1shot1
...
class NasBench101SearchSpace(SearchSpace):
    ...
    def random_sample(self):
        return self.sample(self.loose_end)
    ...
    def _init_nasbench(self):
        ...
        self.base_dir = os.path.join(
            utils.get_mynas_dir("MYNAS_DATA", "data"), "nasbench-101"
        )
...
class NasBench101OneShotSearchSpace(NasBench101SearchSpace):
    ...
    def _is_valid(self, matrix):
        ...
        return matrix.shape[0] == 7 and all([p == k for p, k in zip(self.num_parents, matrix.sum(axis=0))])
    ...
    def sample(self, with_loose_ends, upscale=True):
        if with_loose_ends:
            ...
            #assert self._is_valid(adjacency_matrix_sample), 'incorrect matrix'

        ops = random.choices(self.ops_choices[:-1], k=self.num_intermediate_nodes)
        if upscale and len(self.num_parents_per_node) < 7:
...
class NasBench101EvoController(BaseController):
    ...
    def __init__(
        ...
        # get pickle data
        # base_dir = os.path.join(utils.get_mynas_dir("MYNAS_DATA", "data"), "nasbench-101")
...
class NasBench101EvalArchController(BaseController):
    NAME = "nasbench-101-eval-arch"
    ...
    def __init__(
        ...
        super(NasBench101EvalArchController, self).__init__(
            search_space, rollout_type, mode, schedule_cfg
        )

        with open(arch_file, "r") as fr:
            self.archs = [eval(spec) for spec in yaml.load(fr)]

        self.rollouts = [NasBench101Rollout(spec.original_matrix,
            self.search_space.op_to_idx(spec.ops), self.search_space) for spec in self.archs]
    ...
    def sample(self, n, batch_size=None):
        ...
        if self.mode == "eval":
            ...
            n_evaled = len(self.gt_rollouts)
            if n_evaled < n:
                sampled_rollouts = np.random.choice(self.rollouts, n - n_evaled).tolist()
            best_inds = np.argpartition(self.gt_scores, -n)[-n:]
            return sampled_rollouts + [self.gt_rollouts[ind] for ind in best_inds]

        rollouts = np.random.choice(self.rollouts, n).tolist()
        return rollouts
...
class NasBench101SuperNet(BaseWeightsManager, nn.Module):
    ...
    def step(self, gradients, optimizer):
        ...
        for k, grad in gradients:
            named_params[k.replace("super_net.", "")].grad = grad
        if self.max_grad_norm is not None:
            # clip the gradients
            torch.nn.utils.clip_grad_norm_(self.parameters(), self.max_grad_norm)
        # apply the gradients
...
class NasBench101SuperNetOfficial(BaseWeightsManager, nn.Module):
    NAME = "nasbench-101-official"
    ...
    def __init__(
        ...
        super(NasBench101SuperNetOfficial, self).__init__(search_space, device, rollout_type)
        nn.Module.__init__(self)
        ...
        os.makedirs("./saved_wrapper_2020", exist_ok=True)
        self.wrapper = DartsWrapper("./saved_wrapper_2020", 2020, 256, max_grad_norm, 1000,
                search_space.num_intermediate_nodes,
                search_space=search_space, cutout=None)
    ...
    def forward(self, inputs, genotypes):
        genotypes = (genotypes[0], genotypes[1][1:-1])
        weights = self.wrapper.get_weights_from_arch(genotypes)
        self.wrapper.set_arch_model_weights(weights)
        return self.model(inputs, discrete=True)
